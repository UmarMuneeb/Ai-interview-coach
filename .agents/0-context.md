...
10. **Phase 10 (Dashboard Enhancements)** — Review sidebar showing top 20 incorrectly answered questions fetched from DB; backend endpoint `GET /questions/review` added to `QuestionsService`.
11. **Phase 11 (UI Polish & De-Slopification)** — Removed AI-generated emoji slop from all screens. Replaced with SVG icons, Emil Kowalski easing curves (`cubic-bezier(0.23,1,0.32,1)`), `@media (hover: hover)` guards on all `:hover` rules, skeleton loading states, human copy on login/dashboard, and branded SVG logo in AppLayout.
12. **Phase 12 (Voice Interview Loop Correctness)** — Fixed three root-cause bugs: (a) `ConversationService` now enforces a 1-drill-max state machine (`drillUsed` boolean) so the interviewer never repeats a drill; `processTurn` returns `{text, shouldAdvance, interviewComplete}` instead of a plain string; question advancement no longer clamps at the last question but sets `phase = wrap-up` and emits `interviewComplete`. (b) `VoiceGateway` now injects `SessionsService` and `QuestionsService`; every `voice_turn` persists the answer to DB via `sessionsService.submitAnswer()` and emits `answer_classified` + `question_advanced` events to the client. (c) Frontend hook `useVoiceInterviewer` listens for `question_advanced` and `answer_classified` socket events; interview page wires `onQuestionAdvanced` callback to update question state automatically.
